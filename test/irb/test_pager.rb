...
module TestIRB
  ...
  class PageOverflowIOTest < TestCase
    ...
    def test_callback_delay
      ...
      t=Time.now
      Reline::Unicode.calculate_width('') # warm up
      assert_include(0..0.05,Time.now-t)
      t=Time.now
      Reline::Unicode.calculate_width('a' * 1000)
      assert_include(0..0.05,Time.now-t)

      t=Time.now
      out = IRB::Pager::PageOverflowIO.new(10, 4, overflow_callback, delay: 0.2)
      assert_include(0..0.1,Time.now-t)
      out.write 'a' * 1000
  ...
  [5, 10, 15, 20, 30, 40, 50, 60].each do |n|
  eval <<~RUBY
    def test_callback_delay_#{n}
      actual_events = []
      overflow_callback = ->(lines) do
        actual_events << [:callback_called, lines]
      end
      ...
      t=Time.now
      out = IRB::Pager::PageOverflowIO.new(10, 4, overflow_callback, delay: #{n/100.0})
      assert_include(0..#{n/200.0},Time.now-t)
      out.write 'a' * 1000
      assert_equal ['a' * 10] * 4, out.first_page_lines
      out.write 'b'
      assert_include(0..#{n/200.0},Time.now-t)
